...
namespace App_CentroFitness
{
    public partial class ReprogramarReserva : System.Web.UI.Page
    {
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlNuevaClase.SelectedValue == "0")
                    throw new Exception("Debe seleccionar una clase disponible para reprogramar.");
        ...
        private void cargarClasesDisponibles(int idDisciplina, int idClaseActual, int idAlumno)
        {
            ...
            List<Clase> candidatas = claseNegocio.listarVigentesPorDisciplina(idDisciplina);
            List<Clase> disponibles = new List<Clase>();

            foreach (Clase clase in candidatas)
            {
                if (clase.IdClase == idClaseActual)
                    continue;

                if (reservaNegocio.contarReservasVigentes(clase.IdClase) >= clase.CupoMaximo)
                    continue;

                if (reservaNegocio.existeReserva(idAlumno, clase.IdClase))
                    continue;

                try
                {
                    suscripcionNegocio.validarClaseDentroDeSuscripcion(idAlumno, clase);
                }
                catch
                {
                    continue;
